...
export class BlockchainService {
  ...
  public testTxns: Array<Transaction> = [];
  public randomWallets: Array<IWalletKey> = []; // Creates array of empty random wallets to represent other users
  ...
  minePendingTransactions() {
    this.blockchainInstance.minePendingTransactions(
      this.wallets[Math.floor(Math.random() * 10)].publicKey // Rewards the user's wallet with the mining reward
    );
  }

  // Checks to see if the address passed in is one of the addresses in the user's wallet
  addressIsFromCurrentUser(address) {
    // return this.wallets.filter(function(e) { return e.publicKey === address; }).length > 0;
